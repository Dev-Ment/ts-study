...
위 예시에서는 `func()` 함수의 매개변수에 **number** 타입만 허용되도록 제한되어 있다. 따라서 **string** 타입인 `str`을 인자로 전달하면 에러가 발생한다.

이는 마치 함수가 받아들일 수 있는 값을 **number 타입의 집합**으로 한정하는 것과 같다. 함수의 매개변수를 특정 타입으로 제한함으로써, 코드에서 발생할 수 있는 타입 관련 오류를 사전에 방지할 수 있다.
...
`double()` 함수의 내부 동작을 보면, 숫자를 인자로 받는 것을 기대한다는 것을 알 수 있다.

하지만 이 함수에 숫자가 아닌 다른 타입의 값을 전달하면, 개발자가 의도하지 않은 결과를 얻게 될 수 있다.

아래와 같이 매개변수의 타입을 명시하면, 올바르지 않은 타입의 값으로 함수를 호출했을 때 타입스크립트 컴파일러가 즉시 에러를 발생시킨다.
...
예를 들어, 자바스크립트는 **약타입 언어**로, 타입이 명확하게 잘못 작성된 코드도 암묵적으로 타입 변환을 수행해 결과를 도출한다. 그러나 이러한 동작은 **예기치 못한 오류**를 발생시킬 가능성이 있으므로, **타입 안정성**을 유지하는 것이 중요하다.
...
Cat과 Pet은 서로 다른 타입으로 선언되었지만, Pet 타입이 요구하는 `name` 속성을 Cat 타입도 포함하고 있기 때문에, Cat 타입으로 선언된 `cat`을 Pet 타입으로 선언된 `pet`에 할당할 수 있다.

</details>

<details>
  <summary>구조적 서브 타이핑은 함수의 매개변수에도 적용된다.</summary>
...
`greet()` 함수의 매개변수에 들어갈 수 있는 값은 **Pet** 타입으로 제한되어 있지만, **cat** 객체는 여전히 인자로 전달할 수 있다.

왜냐하면 `cat` 객체는 **Pet** 인터페이스가 요구하는 `name` 속성을 가지고 있기 때문이다. 따라서 `pet.name` 방식으로 `name` 속성에 접근할 수 있다.

</details>

<details>
  <summary>구조적 서브 타이핑은 타입의 상속에도 적용된다.</summary>
...
`Developer` 클래스가 `Person` 클래스를 상속받지 않아도, `Developer`가 `Person`이 가진 속성을 모두 가지고 있기 때문에 `greet()` 함수에 `Developer` 객체를 인자로 전달할 수 있다.
...
모든 변수와 표현식의 타입을 컴파일타임에 검사하지 않아도 되기 때문에 타입이 올바르게 정해지지 않으면 런타임에서 에러가 발생하기도 한다. 즉, 타입의 정확성을 100% 보장해주지 않는다.
...
그러나 자바스크립트의 프로토타입 체이닝 때문에 기본 Object 객체의 메서드(`toString`)는 사용할 수 있다.
