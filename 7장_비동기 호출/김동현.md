...
위 코드는 주문 내역을 요청할 때 에러가 발생하면 에러 메세지를 얼럿을 사용하여 사용자에게 표시해준다. 이때 "로그인 정보가 만료되었습니다." , "유효하지 않은 요청 데이터입니다." 와 같이 서버에서 전달된 에러 메세지를 보고 사용자는 어떤 에러가 발생한 것인지 판단할 수 있더라도 개발자는 사용자 로그인 정보가 만료되었는지, 타임아웃이 발생한건지 구분할 수 없다.
